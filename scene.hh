...
namespace mmog {
  ...
  inline Scene::Scene(Map && map, Actors && actors)
    ...
    , actors(std::make_move_iterator(std::begin(actors)), std::make_move_iterator(std::end(actors))) {
  }

  inline char const & Scene::move(point & pt, Action_Direction const & dir) const {
    switch (dir)
    {
    case Action_Direction::Up:    pt.y--; break;
    case Action_Direction::Left:  pt.x--; break;
    case Action_Direction::Down:  pt.y++; break;
    case Action_Direction::Right: pt.x++; break;
    }
  ...
  inline void Scene::update() {
    for (auto && actor : actors) {
      ...
      while (!actor->empty()) {
        ...
        switch (action.tag())
        {
        case Action::Tag::None: break;
        case Action::Tag::Move: this->move(actor->position(), action.get<Action::Tag::Move>()); break;
        }
      }
    }
  }
  ...
  inline void Scene::draw(std::vector<char> & buffer) const {
    for (auto y = 0, height = this->height(); y < height; y++) for (auto x = 0, width = this->width(); x < width; x++)
  ...
  inline int Scene::width() const {
    return this->map->width();
  }
  inline int Scene::height() const {
