...
struct Exit {
  enum class Function { Load, Jump } function;
  std::string name;
  enum class Direction { None, Up, Left, Down, Right } direction;
};
...
std::unique_ptr<Scene> loadLevel(std::string const &name) {
  auto level = std::find_if(std::begin(levels),
                            std::end(levels),
                            [&name](auto const &l) { return name == l.name; });
  if (level == std::end(levels)) throw_unknown_level_error(name);
  ...
  return std::make_unique<Scene>(
      std::make_unique<Map>(map, level->width, level->height),
      std::move(actors));
}
}
